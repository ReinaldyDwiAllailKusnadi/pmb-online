...
<header class="fixed right-0 top-0 z-40 flex h-20 w-[calc(100%-260px)] items-center justify-between border-b border-slate-100 bg-white/80 px-10 backdrop-blur-md">
    <div class="flex max-w-xl flex-1 items-center">
        @if ($showSearch)
            <div class="group relative w-full">
                <i class="bi bi-search absolute left-4 top-1/2 -translate-y-1/2 text-xl leading-none text-slate-400 transition-colors group-focus-within:text-primary"></i>
                <input type="text"
    ...
    <div class="ml-6 flex items-center gap-4">
        <button type="button" class="group relative rounded-xl p-3 text-primary transition-all hover:bg-slate-50" aria-label="Notifikasi">
            <i class="bi bi-bell-fill text-xl leading-none"></i>
            <span class="absolute right-3 top-3 h-2.5 w-2.5 animate-pulse rounded-full border-2 border-white bg-red-500 ring-2 ring-red-500/20"></span>
        </button>
        <button type="button" class="rounded-xl p-3 text-primary transition-all hover:bg-slate-50" aria-label="Bantuan">
